...
export class BarangayListPage implements OnInit, OnDestroy, OnToolbarItemClick {
  columnDefs: ColDef[] = [
  ...
  ngOnInit() {
    ...
    this.items$ = this.query.selectAll({
      filterBy: a =>
        a.areaClassifications.findIndex(i => i.categoryType === 'Barangay') >=
        0,
    });
    this.service.findAll('Barangay').subscribe();
  }
