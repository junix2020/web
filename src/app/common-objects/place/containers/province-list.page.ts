...
export class ProvinceListPage implements OnInit, OnDestroy, OnToolbarItemClick {
  columnDefs: ColDef[] = [
  ...
  ngOnInit() {
    ...
    this.items$ = this.query.selectAll({
      filterBy: a =>
        a.areaClassifications.findIndex(i => i.categoryType === 'Province') >=
        0,
    });
    this.service.findAll('Province').subscribe();
  }
