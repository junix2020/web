...
import { formEditToolbars, OnToolbarItemClick, Toolbar, ToolbarItemType, ToolbarType } from '@web/shell';
    ...
    <form [formGroup]="form">
      <nz-collapse>
        <nz-collapse-panel
          ...
          [nzDisabled]="true"
        >
          <app-province-form formControlName="province"></app-province-form>
        </nz-collapse-panel>
      </nz-collapse>
      <nz-collapse>
        <nz-collapse-panel
...
export class ProvinceEditPage implements OnInit, OnDestroy, OnToolbarItemClick {
  form: FormGroup;
  editFlag = false;
  destroy$ = new Subject();
  ...
  enableSaveToolbar() {
    ...
    if (this.form.value.province.status === 'Draft') {
  ...
  populateForm() {
    ...
    this.form.controls.province.patchValue(area);
  ...
  onSave(status = 'Active') {
    ...
    const area = { ...province, status } as Area;
    this.service.save(area).subscribe(() => {
      this.form.controls.province.patchValue({ status });
