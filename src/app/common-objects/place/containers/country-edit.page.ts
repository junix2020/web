...
export class CountryEditPage implements OnInit, OnDestroy, OnToolbarItemClick {
  form: FormGroup;
  editFlag = false;
  destroy$ = new Subject();
  ...
  enableSaveToolbar() {
    ...
    if (this.form.value.country.status === 'Draft') {
  ...
  populateForm() {
    ...
    this.form.controls.country.patchValue(area);
  ...
  onSave(status = 'Active') {
    ...
    this.service.save(area).subscribe(() => {
      this.form.controls.country.patchValue({ status });
