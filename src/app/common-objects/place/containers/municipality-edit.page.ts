...
export class MunicipalityEditPage
  ...
  enableSaveToolbar() {
    ...
    if (this.form.value.municipality.status === 'Draft') {
  ...
  populateForm() {
    ...
    this.form.controls.municipality.patchValue(area);
  ...
  onSave(status = 'Active') {
    ...
    const area = { ...municipality, status } as Area;
    this.service.save(area).subscribe(() => {
      this.form.controls.municipality.patchValue({ status });
