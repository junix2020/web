...
@Component({
  selector: 'app-role-type-list-page',
  template: `
    <nz-collapse>
      <nz-collapse-panel
        nzHeader="Role Type"
...
export class RoleTypeListPage {
  ...
  onToolbarItemClicked(type: ToolbarItemType) {
    switch (type) {
      ...
      case ToolbarType.VIEW:
        this.router.navigate([this.selectedItems[0].roleTypeID], {
      ...
      case ToolbarType.DELETE:
        this.modal.confirm({
          ...
          nzOnOk: () =>
            this.service
              .deleteMany(this.selectedItems.map(a => a.roleTypeID))
              .subscribe(),
          nzCancelText: 'No',
        });
        break;
  ...
  onDoubleClick(item: RoleType) {
    this.router.navigate([item.roleTypeID], { relativeTo: this.route });
  }
}
