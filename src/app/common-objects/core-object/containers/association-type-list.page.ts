...
@Component({
  selector: 'app-association-type-list-page',
  template: `
    <nz-collapse>
      <nz-collapse-panel
        nzHeader="Association Type"
        [nzActive]="true"
        [nzShowArrow]="false"
        [nzDisabled]="true"
      >
...
export class AssociationTypeListPage {
  columnDefs: ColDef[] = [
  ...
  selectedItems: AssociationType[];
  items$: Observable<AssociationType[]>;
  destroy$ = new Subject();
  ...
  onToolbarItemClicked(type: ToolbarItemType) {
    switch (type) {
      ...
      case ToolbarType.VIEW:
        this.router.navigate([this.selectedItems[0].associationTypeID], {
      ...
      case ToolbarType.DELETE:
        this.modal.confirm({
          ...
          nzOnOk: () =>
            this.service
              .deleteMany(this.selectedItems.map(a => a.associationTypeID))
              .subscribe(),
          nzCancelText: 'No',
        });
        break;
  ...
  onRowSelect(items: AssociationType[]) {
  ...
  onDoubleClick(item: AssociationType) {
    this.router.navigate([item.associationTypeID], { relativeTo: this.route });
  }
}
