import { Component, ChangeDetectionStrategy, OnInit, OnDestroy, Input, Output, EventEmitter, ChangeDetectorRef, HostListener } from '@angular/core';
...
export class OrganizationsListComponent implements OnInit, OnDestroy {
  ...
  @Input() organizations: OrganizationsListDTO[];
  ...
  selectedItems: OrganizationsListDTO[];
  ...
  constructor(
    ...
    private store: OrganizationsListStore,
    ...
    gridOptions: {
      this.columnDefs = [
        ...
        { headerName: 'Official Name', field: 'officialName', hide: false },
  ...
  // method to delete the party in the list, cancel if status is active
  onDeleteParty(): void {
    this.partiesService.deletePartyByIDs(this.selectedItems.map(p => p.partyID), 'organization')
    this.onRemoveSelected();
  }

  // method to detect selection change to enable/disable toolbar
  onSelectionChanged(organizations: OrganizationsListDTO[]) {
  ...
  updateCities() {
    this.organizations = [];
    this.organizationsListQuery.selectAll().subscribe(data => {
