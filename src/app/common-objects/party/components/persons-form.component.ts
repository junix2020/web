...
<!-- [nzNoColon]="true" -->
<form nz-form  [nzNoColon]="true" [formGroup]="personForm" novalidate >
   
         <nz-form-item>
            <nz-form-label [nzSm]="6" [nzXs]="24" nzFor="personCategoryTypeName">Party Type</nz-form-label>
            <nz-form-control [nzSm]="16" [nzXs]="24">
               <input readonly formControlName="personCategoryTypeName" nz-input id="personCategoryTypeName"/>
            </nz-form-control>
         </nz-form-item>
         ...
         <nz-form-item>
            <nz-form-label [nzSm]="6" [nzXs]="24" nzFor="middleName">Middle Name</nz-form-label>
            <nz-form-control [nzSm]="16" [nzXs]="24">
         ...
         <nz-form-item>
             <nz-form-label [nzSm]="6" [nzXs]="24" nzFor="birthDate">Birth Date</nz-form-label>
                <nz-form-control [nzSm]="16" [nzXs]="24">
                    <nz-date-picker input="date" [nzFormat] formControlName="birthDate" id="birthDate"></nz-date-picker>
                </nz-form-control>
         </nz-form-item>
            ...
            <input nz-input formControlName="personCategoryTypeID" id="personCategoryTypeID" type="hidden"/> 
...
export class PersonsFormComponent implements OnInit, AfterViewInit, OnDestroy {

 // these events are called when there is and need to execute a method in the parent component
  @Output() onSave = new EventEmitter<PersonsFormDTO>();  // use in New, View and Edit
  ...
  classifications: any[] = [];
  //classifications: Array<PersonClassificationDTO>;
  ...
  setDeleteToolbar() {
    var status = this.personForm.get('statusName').value;
  ...
  onEmptyForm() {
    this.formsManager.remove('personsForm');
  }

  // method to save and submit city area data
  savePerson() {
    this.onSave.emit(this.formsManager.getForm('personsForm').value);
  }
  ...
  formReset() {
    ...
    this.formsManager.unsubscribe('personsForm');
    // reset person form
  ...
  picklistSource() {
    ...
    switch (this.picklistName) {
      ...
      case 'Status':
        ...
        this.onUpdatePersonStatus(this.itemsParse);
        break;
      default:
        break;
    }
  ...
  displayRetrievedData(params, isMsg) {
    ...
    person.classifications = params.classifications;
  ...
  buildForm() {
    ...
    this.personForm = this.fb.group({
      personCategoryTypeID: ['88666a3d-92e1-4f44-8a27-99a3c2c0dd3f'],
      personCategoryTypeName: ['Person'],
      partyID: [partyId],
      code: null,
      name: null,
      description: null,
      ...
      this.formsManager.upsert('personsForm', this.personForm, {
 ...
 // method to display retrieved city into form
  displayPerson(person) {
    this.personForm.patchValue({
      personCategoryTypeID: person.personCategoryTypeID,
      personCategorytypeName: person.personCategoryTypeName,
      partyID: person.partyID,
      code: person.code,
      name: person.name,
      description: person.description,
      permanentRecordIndicator: person.permanentRecordIndicator,
      colorID: person.colorID,
      personPartyID: person.personPartyID,
      firstName: person.firstName,
      middleName: person.middleName,
      lastName: person.lastName,
      birthDate: person.birthDate,
      partyStatusID: person.partyStatusID,
      startDateTime: person.startDateTime,
      endDateTime: person.endDateTime,
      partyStatusPartyID: person.partyStatusPartyID,
    ...
    this.personForm.setControl('classifications', this.setExistingClassifications(person.classifications));
    this.setPartiesClassificationList();
    this.ref.detectChanges();
  }
  ...
  onAddPartiesClassifcation(classifications: any[]): void {
    ...
    if (classifications.length < 1) {
     ...
     let partyClassifications = this.personForm.controls['classifications'] as FormArray;
      classifications.map(c => {
        ...
        if (idx < 0) {
            ...
            partyID: this.personForm.get('partyID').value,
  ...
  get getClassifier(): FormArray {
    return this.personForm.get('classifications') as FormArray;
  }
  ...
  setPartiesClassificationList() {
    ...
    this.classifications = this.personForm.get('classifications').value;
  ...
  onUpdatePersonStatus(status: any[]) {
    if (status.length < 1) {
      return;
    }
    status.map(s => {
      // assign values to status control
      this.personForm.get('statusTypeID').patchValue(s.statusTypeId);
      this.personForm.get('statusName').patchValue(s.name);
    })
  ...
  onAddClassification() {
     this.router.navigateByUrl('/common-objects/party/person/classification-picklist', { state: { source: this.sourceParty } });
  }

  // method to call area status type picklist using the navigating url
  onPickStatusType() {
    this.router.navigateByUrl('/common-objects/party/person/status-picklist', { state: { source: this.sourceParty } });
...
} // personsFormCompoent end brace pair
